...
export async function fetchChatServiceFriendsOverviewFromSpring(currentProfileId: string) {
  const url = new URL(`${resolveSpringBackendBaseUrl()}/chat-service/friends/overview`);
  ...
  if (!response.ok) {
    throw new ChatServiceFriendsOverviewSpringBackendHttpError(response.status, extractErrorMessage(parsed) ?? "Spring backend 요청에 실패했습니다.");
  }
  return parsed as { friends: unknown[]; pendingSent: unknown[]; pendingReceived: unknown[]; suggested: unknown[]; blocked: unknown[] };
}
