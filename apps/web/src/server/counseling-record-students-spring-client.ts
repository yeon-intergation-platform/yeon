...
export class CounselingRecordStudentsSpringBackendHttpError extends Error {
  readonly status: number;
  constructor(status: number, message: string) {
    super(message);
    this.name = "CounselingRecordStudentsSpringBackendHttpError";
    this.status = status;
  }
}
...
export async function fetchCounselingRecordStudentsFromSpring(userId: string) {
  const response = await fetch(`${resolveSpringBackendBaseUrl()}/counseling-records/students`, {
    method: "GET",
    cache: "no-store",
    headers: {
  ...
  if (!response.ok) {
    throw new CounselingRecordStudentsSpringBackendHttpError(response.status, extractErrorMessage(parsed) ?? "Spring backend 요청에 실패했습니다.");
  }
  return parsed as { students: Array<{ studentName: string; recordCount: number; firstCounselingAt: string; lastCounselingAt: string; }>; };
}
