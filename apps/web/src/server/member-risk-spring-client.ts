...
export class MemberRiskSpringBackendHttpError extends Error {
  readonly status: number;
  constructor(status: number, message: string) {
    super(message);
    this.name = "MemberRiskSpringBackendHttpError";
    this.status = status;
  }
}
...
export async function fetchMemberRiskProfilesFromSpring(params: {
  userId: string;
  members: Array<{ id: string; initialRiskLevel?: string | null }>;
}) {
  const response = await fetch(`${resolveSpringBackendBaseUrl()}/member-risk-profiles`, {
    method: "POST",
    cache: "no-store",
    headers: {
  ...
  if (!response.ok) {
    throw new MemberRiskSpringBackendHttpError(
      response.status,
