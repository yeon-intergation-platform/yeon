import type { MergeGuestRequest, MergeGuestResponse } from "@yeon/api-contract/card-deck-merge-guest";
...
export async function mergeGuestCardDecksInSpring(params: {
  ...
  const response = await fetch(`${resolveSpringBackendBaseUrl()}/card-decks/merge-guest`, {
    method: "POST",
    cache: "no-store",
    headers: {
