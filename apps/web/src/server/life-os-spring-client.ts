...
export async function fetchLifeOsDayFromSpring(userId: string, localDate: string) {
  return fetchJson(`/life-os/days/${encodeURIComponent(localDate)}`, userId) as Promise<{ day: unknown }>;
}
...
export async function updateLifeOsDayInSpring(userId: string, localDate: string, body: unknown) {
...
export async function fetchLifeOsDailyReportFromSpring(userId: string, localDate: string) {
  return fetchJson(`/life-os/reports/daily?localDate=${encodeURIComponent(localDate)}`, userId) as Promise<{ report: unknown }>;
}

export async function fetchLifeOsWeeklyReportFromSpring(userId: string, periodStart: string, periodEnd: string) {
  const query = new URLSearchParams({ periodStart, periodEnd });
  return fetchJson(`/life-os/reports/weekly?${query.toString()}`, userId) as Promise<{ report: unknown }>;
}
