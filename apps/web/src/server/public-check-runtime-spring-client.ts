...
export async function fetchPublicCheckSessionFromSpring(params: {
  ...
  return fetchJson(
    `/public-check-sessions/${params.token}${search.size > 0 ? `?${search.toString()}` : ""}`
  ) as Promise<{
    spaceId: string;
    session: unknown;
...
export async function verifyPublicCheckIdentityInSpring(
  token: string,
  body: { name: string; phoneLast4: string }
) {
...
export async function submitPublicCheckInSpring(
  token: string,
  body: {
    method: string;
    name?: string | null;
