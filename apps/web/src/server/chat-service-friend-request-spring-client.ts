...
export async function sendChatServiceFriendRequestInSpring(params: { currentProfileId: string; targetProfileId: string; }) {
  const url = new URL(`${resolveSpringBackendBaseUrl()}/chat-service/friends/requests`);
  ...
  if (!response.ok) {
    throw new ChatServiceFriendRequestSpringBackendHttpError(response.status, extractErrorMessage(parsed) ?? "Spring backend 요청에 실패했습니다.");
  }
