...
export class CounselingRecordListSpringBackendHttpError extends Error {
  readonly status: number;
  constructor(status: number, message: string) {
    super(message);
    this.name = "CounselingRecordListSpringBackendHttpError";
    this.status = status;
  }
}
...
export async function fetchCounselingRecordListFromSpring(params: {
  userId: string;
  spaceId?: string | null;
  unlinked?: boolean;
  limit?: number;
  ...
  const url = new URL(`${resolveSpringBackendBaseUrl()}/counseling-records`);
  if (params.spaceId) url.searchParams.set("spaceId", params.spaceId);
  if (params.unlinked) url.searchParams.set("unlinked", "true");
  ...
  if (!response.ok) {
    throw new CounselingRecordListSpringBackendHttpError(
      response.status,
