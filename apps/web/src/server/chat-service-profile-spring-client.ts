...
export async function fetchChatServiceProfileFromSpring(params: {
  currentProfileId: string;
  targetProfileId: string;
}) {
  const url = new URL(`${resolveSpringBackendBaseUrl()}/chat-service/profiles/${params.targetProfileId}`);
  ...
  if (!response.ok) {
    throw new ChatServiceProfileSpringBackendHttpError(response.status, extractErrorMessage(parsed) ?? "Spring backend 요청에 실패했습니다.");
  }
