...
async function send(params: {
  ...
  const url = createYeonUrl(
    `${resolveSpringBackendBaseUrl()}/chat-service/profiles/${params.targetProfileId}/block`
  );
  ...
  if (!response.ok) {
    throw new ChatServiceBlockSpringBackendHttpError(
      response.status,
...
export function blockChatServiceProfileInSpring(
  currentProfileId: string,
  targetProfileId: string
) {
...
export function unblockChatServiceProfileInSpring(
  currentProfileId: string,
  targetProfileId: string
) {
