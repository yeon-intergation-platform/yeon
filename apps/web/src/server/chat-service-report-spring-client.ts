...
export async function createChatServiceReportInSpring(params: { currentProfileId: string; targetType: string; targetId: string; reason: string; }) {
  const url = new URL(`${resolveSpringBackendBaseUrl()}/chat-service/reports`);
  const response = await fetch(url, {
    ...
    body: JSON.stringify({ targetType: params.targetType, targetId: params.targetId, reason: params.reason }),
  });
  const raw = await response.text();
  const parsed = tryParseJson(raw);
  if (!response.ok) throw new ChatServiceReportSpringBackendHttpError(response.status, extractErrorMessage(parsed) ?? "Spring backend 요청에 실패했습니다.");
