...
export class ChatServiceChatOpenSpringBackendHttpError extends Error {
  readonly status: number;
  constructor(status: number, message: string) {
    super(message);
    this.name = "ChatServiceChatOpenSpringBackendHttpError";
    this.status = status;
  }
}
...
export async function openChatServiceRoomInSpring(params: { currentProfileId: string; targetProfileId: string; }) {
  const url = new URL(`${resolveSpringBackendBaseUrl()}/chat-service/chat/open`);
  ...
  if (!response.ok) {
    throw new ChatServiceChatOpenSpringBackendHttpError(response.status, extractErrorMessage(parsed) ?? "Spring backend 요청에 실패했습니다.");
  }
  return parsed as { room: unknown };
}
