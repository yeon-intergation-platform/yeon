...
export async function openChatServiceRoomInSpring(params: {
  currentProfileId: string;
  targetProfileId: string;
}) {
  const url = createYeonUrl(
    `${resolveSpringBackendBaseUrl()}/chat-service/chat/open`
  );
  ...
  if (!response.ok) {
    throw new ChatServiceChatOpenSpringBackendHttpError(
      response.status,
