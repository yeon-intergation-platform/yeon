...
async function fetchSpring(path: string, init: RequestInit, fallback: string) {
  ...
  if (!response.ok) {
    throw new ChatServiceChatRoomsSpringBackendHttpError(response.status, extractErrorMessage(parsed) ?? fallback);
  }
  return parsed;
}

export async function fetchChatServiceRoomsFromSpring(currentProfileId: string) {
  return fetchSpring("/chat-service/chat/rooms", {
    method: "GET",
    headers: { [CHAT_PROFILE_HEADER]: currentProfileId },
  }, "Spring backend 요청에 실패했습니다.") as Promise<{ rooms: unknown[] }>;
}

export async function fetchChatServiceRoomFromSpring(params: { currentProfileId: string; roomId: string; }) {
  return fetchSpring(`/chat-service/chat/rooms/${params.roomId}`, {
    method: "GET",
    headers: { [CHAT_PROFILE_HEADER]: params.currentProfileId },
  }, "Spring backend 요청에 실패했습니다.") as Promise<{ room: unknown; messages: unknown[] }>;
}

export async function sendChatServiceMessageInSpring(params: { currentProfileId: string; roomId: string; body: string; }) {
  return fetchSpring(`/chat-service/chat/rooms/${params.roomId}/messages`, {
    method: "POST",
    headers: {
  ...
  }, "Spring backend 요청에 실패했습니다.") as Promise<{ message: unknown }>;
}
