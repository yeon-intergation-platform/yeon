...
import { fetchOwnedMemberFromSpring, MembersSpringBackendHttpError } from "@/server/members-spring-client";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ memberId: string }> }) {
  ...
  try {
    const { member } = await fetchOwnedMemberFromSpring(memberId, currentUser.id);
    ...
    console.error(error); return jsonError("수강생 정보를 불러오지 못했습니다.", 500);
  }
}
