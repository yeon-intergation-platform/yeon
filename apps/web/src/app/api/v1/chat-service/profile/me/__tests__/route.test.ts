...
vi.mock("@/app/api/v1/chat-service/_shared", () => ({
  clearChatServiceSessionCookie: (...args: unknown[]) =>
    mockClearChatServiceSessionCookie(...args),
...
vi.mock("@/server/chat-service-my-profile-spring-client", async () => {
  const actual = await vi.importActual<
    typeof import("@/server/chat-service-my-profile-spring-client")
  >("@/server/chat-service-my-profile-spring-client");
  return {
    ...actual,
    fetchMyChatServiceProfileFromSpring: (...args: unknown[]) =>
      mockFetchMyChatServiceProfileFromSpring(...args),
    updateMyChatServiceProfileInSpring: (...args: unknown[]) =>
      mockUpdateMyChatServiceProfileInSpring(...args),
    deleteMyChatServiceProfileInSpring: (...args: unknown[]) =>
      mockDeleteMyChatServiceProfileInSpring(...args),
  };
});
...
describe("chat-service my profile route", () => {
  ...
  it("Spring 프로필 응답을 반환한다", async () => {
    ...
    mockFetchMyChatServiceProfileFromSpring.mockResolvedValue({
      profile: {
        id: "11111111-1111-4111-8111-111111111111",
        phoneNumberMasked: "010-12**-7890",
        nickname: "닉",
        ageLabel: "20세",
        regionLabel: "서울",
        avatarUrl: null,
    ...
    const response = await GET(
      new NextRequest("http://localhost/api/v1/chat-service/profile/me")
    );
  ...
  it("Spring 프로필 수정 응답을 반환한다", async () => {
    ...
    mockUpdateMyChatServiceProfileInSpring.mockResolvedValue({
      profile: {
        id: "11111111-1111-4111-8111-111111111111",
        phoneNumberMasked: "010-12**-7890",
        nickname: "닉2",
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockFetchMyChatServiceProfileFromSpring.mockRejectedValue(
      new ChatServiceMyProfileSpringBackendHttpError(
        404,
        "프로필을 찾지 못했습니다."
      )
    );
