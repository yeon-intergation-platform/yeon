...
vi.mock("@/server/chat-service-block-spring-client", async () => {
  const actual = await vi.importActual<
    typeof import("@/server/chat-service-block-spring-client")
  >("@/server/chat-service-block-spring-client");
  return {
    ...actual,
    blockChatServiceProfileInSpring: (...args: unknown[]) =>
      mockBlockChatServiceProfileInSpring(...args),
    unblockChatServiceProfileInSpring: (...args: unknown[]) =>
      mockUnblockChatServiceProfileInSpring(...args),
  };
});
...
describe("chat-service profile block route", () => {
  ...
  it("POST는 Spring 차단 응답을 반환한다", async () => {
    ...
    mockBlockChatServiceProfileInSpring.mockResolvedValue({
      blockedProfiles: [],
    });

    const response = await POST(
      new NextRequest(
        "http://localhost/api/v1/chat-service/profiles/22222222-2222-4222-8222-222222222222/block",
        { method: "POST" }
      ),
      {
  ...
  it("DELETE는 Spring 차단해제 응답을 반환한다", async () => {
    ...
    mockUnblockChatServiceProfileInSpring.mockResolvedValue({
      blockedProfiles: [],
    });
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockBlockChatServiceProfileInSpring.mockRejectedValue(
      new ChatServiceBlockSpringBackendHttpError(
        404,
        "차단 대상 프로필을 찾지 못했습니다."
      )
    );
