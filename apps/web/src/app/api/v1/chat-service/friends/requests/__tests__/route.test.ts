...
vi.mock("@/server/chat-service-friend-request-spring-client", async () => {
  const actual = await vi.importActual<typeof import("@/server/chat-service-friend-request-spring-client")>("@/server/chat-service-friend-request-spring-client");
  return {
    ...actual,
    sendChatServiceFriendRequestInSpring: (...args: unknown[]) => mockSendChatServiceFriendRequestInSpring(...args),
  };
});
...
describe("chat-service friend request route", () => {
  ...
  it("invalid body면 400을 반환한다", async () => {
    ...
    const response = await POST(new NextRequest("http://localhost/api/v1/chat-service/friends/requests", { method: "POST" }));
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockSendChatServiceFriendRequestInSpring.mockRejectedValue(
      new ChatServiceFriendRequestSpringBackendHttpError(404, "친구 요청 대상 프로필을 찾지 못했습니다."),
    );
