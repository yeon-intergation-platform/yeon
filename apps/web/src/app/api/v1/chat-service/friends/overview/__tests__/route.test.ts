...
vi.mock("@/server/chat-service-friends-overview-spring-client", async () => {
  const actual = await vi.importActual<
    typeof import("@/server/chat-service-friends-overview-spring-client")
  >("@/server/chat-service-friends-overview-spring-client");
  return {
    ...actual,
    fetchChatServiceFriendsOverviewFromSpring: (...args: unknown[]) =>
      mockFetchChatServiceFriendsOverviewFromSpring(...args),
  };
});
...
describe("chat-service friends overview route", () => {
  ...
  it("Spring overview 응답을 반환한다", async () => {
    ...
    const response = await GET(
      new NextRequest("http://localhost/api/v1/chat-service/friends/overview")
    );

    expect(mockFetchChatServiceFriendsOverviewFromSpring).toHaveBeenCalledWith(
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockFetchChatServiceFriendsOverviewFromSpring.mockRejectedValue(
      new ChatServiceFriendsOverviewSpringBackendHttpError(
        403,
        "권한이 없습니다."
      )
    );
