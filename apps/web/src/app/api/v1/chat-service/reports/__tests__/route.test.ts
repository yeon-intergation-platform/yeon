...
vi.mock("@/server/chat-service-report-spring-client", async () => {
  const actual = await vi.importActual<
    typeof import("@/server/chat-service-report-spring-client")
  >("@/server/chat-service-report-spring-client");
  return {
    ...actual,
    createChatServiceReportInSpring: (...args: unknown[]) =>
      mockCreateChatServiceReportInSpring(...args),
  };
});
...
describe("chat-service reports route", () => {
  ...
  it("invalid body면 400을 반환한다", async () => {
    ...
    const response = await POST(
      new NextRequest("http://localhost/api/v1/chat-service/reports", {
        method: "POST",
      })
    );
  ...
  it("Spring 신고 응답을 반환한다", async () => {
    ...
    mockCreateChatServiceReportInSpring.mockResolvedValue({
      report: {
    ...
    expect(mockCreateChatServiceReportInSpring).toHaveBeenCalledWith({
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockCreateChatServiceReportInSpring.mockRejectedValue(
      new ChatServiceReportSpringBackendHttpError(
        404,
        "신고 대상 프로필을 찾지 못했습니다."
      )
    );
