...
vi.mock("@/server/chat-service-ask-spring-client", async () => {
  ...
  return {
    ...actual,
    voteChatServiceAskPostInSpring: (...args: unknown[]) =>
      mockVoteChatServiceAskPostInSpring(...args),
  };
});
...
describe("chat-service ask vote route", () => {
  ...
  it("invalid body면 400을 반환한다", async () => {
    ...
    const response = await POST(
      new NextRequest("http://localhost/api/v1/chat-service/ask/333/vote", {
        method: "POST",
      }),
      {
  ...
  it("Spring 투표 응답을 반환한다", async () => {
    ...
    mockParseJsonBody.mockResolvedValue({ optionIndex: 1 });
    mockVoteChatServiceAskPostInSpring.mockResolvedValue({
      post: {
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockParseJsonBody.mockResolvedValue({ optionIndex: 1 });
    mockVoteChatServiceAskPostInSpring.mockRejectedValue(
