...
vi.mock("@/server/chat-service-ask-spring-client", async () => {
  const actual = await vi.importActual<
    typeof import("@/server/chat-service-ask-spring-client")
  >("@/server/chat-service-ask-spring-client");
  return {
    ...actual,
    fetchChatServiceAskPostsFromSpring: (...args: unknown[]) =>
      mockFetchChatServiceAskPostsFromSpring(...args),
    createChatServiceAskPostInSpring: (...args: unknown[]) =>
      mockCreateChatServiceAskPostInSpring(...args),
  };
});
...
describe("chat-service ask route", () => {
  ...
  it("Spring ask 목록 응답을 반환한다", async () => {
    ...
    const response = await GET(
      new NextRequest("http://localhost/api/v1/chat-service/ask")
    );
  ...
  it("Spring ask 생성 응답을 반환한다", async () => {
    ...
    mockCreateChatServiceAskPostInSpring.mockResolvedValue({
      post: {
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockFetchChatServiceAskPostsFromSpring.mockRejectedValue(
      new ChatServiceAskSpringBackendHttpError(
        500,
        "에스크 목록을 불러오지 못했습니다."
      )
    );
