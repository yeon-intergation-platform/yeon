...
vi.mock("@/server/chat-service-chat-rooms-spring-client", async () => {
  const actual = await vi.importActual<
    typeof import("@/server/chat-service-chat-rooms-spring-client")
  >("@/server/chat-service-chat-rooms-spring-client");
  return {
    ...actual,
    fetchChatServiceRoomsFromSpring: (...args: unknown[]) =>
      mockFetchChatServiceRoomsFromSpring(...args),
  };
});
...
describe("chat-service chat rooms route", () => {
  ...
  it("Spring 목록 응답을 반환한다", async () => {
    ...
    const response = await GET(
      new NextRequest("http://localhost/api/v1/chat-service/chat/rooms")
    );
    expect(mockFetchChatServiceRoomsFromSpring).toHaveBeenCalledWith(
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockFetchChatServiceRoomsFromSpring.mockRejectedValue(
      new ChatServiceChatRoomsSpringBackendHttpError(
        500,
        "대화방 목록을 불러오지 못했습니다."
      )
    );
