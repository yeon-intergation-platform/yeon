...
vi.mock("@/server/chat-service-chat-open-spring-client", async () => {
  const actual = await vi.importActual<
    typeof import("@/server/chat-service-chat-open-spring-client")
  >("@/server/chat-service-chat-open-spring-client");
  return {
    ...actual,
    openChatServiceRoomInSpring: (...args: unknown[]) =>
      mockOpenChatServiceRoomInSpring(...args),
  };
});
...
describe("chat-service chat open route", () => {
  ...
  it("invalid body면 400을 반환한다", async () => {
    ...
    const response = await POST(
      new NextRequest("http://localhost/api/v1/chat-service/chat/open", {
        method: "POST",
      })
    );
