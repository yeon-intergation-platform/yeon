...
vi.mock("@/server/chat-service-chat-rooms-spring-client", async () => {
  ...
  return {
    ...actual,
    sendChatServiceMessageInSpring: (...args: unknown[]) =>
      mockSendChatServiceMessageInSpring(...args),
  };
});
...
describe("chat-service send message route", () => {
  ...
  it("invalid body면 400을 반환한다", async () => {
    ...
    const response = await POST(
      new NextRequest(
        "http://localhost/api/v1/chat-service/chat/rooms/333/messages",
        { method: "POST" }
      ),
      {
  ...
  it("Spring 메시지 응답을 반환한다", async () => {
    ...
    mockSendChatServiceMessageInSpring.mockResolvedValue({
      message: {
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockParseJsonBody.mockResolvedValue({ body: "안녕" });
    mockSendChatServiceMessageInSpring.mockRejectedValue(
