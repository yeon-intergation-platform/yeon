...
vi.mock("@/server/chat-service-chat-rooms-spring-client", async () => {
  ...
  return {
    ...actual,
    fetchChatServiceRoomFromSpring: (...args: unknown[]) =>
      mockFetchChatServiceRoomFromSpring(...args),
  };
});
...
describe("chat-service chat room detail route", () => {
  ...
  it("Spring 상세 응답을 반환한다", async () => {
    ...
    const response = await GET(
      new NextRequest("http://localhost/api/v1/chat-service/chat/rooms/333"),
      {
  ...
  it("Spring 오류를 그대로 반환한다", async () => {
    ...
    mockFetchChatServiceRoomFromSpring.mockRejectedValue(
