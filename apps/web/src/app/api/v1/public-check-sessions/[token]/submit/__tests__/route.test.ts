...
vi.mock("@/server/public-check-device-cookie-bff", () => ({
  ...
  getRememberedPublicCheckIdentities: vi.fn(() => [
    { spaceId: "space-1", memberId: "member-1" },
  ]),
}));
...
describe("/api/v1/public-check-sessions/[token]/submit", () => {
  ...
  test("POST: Spring submit을 호출하고 result 응답을 유지한다", async () => {
    ...
    const response = await POST(
      new Request(
        "http://localhost/api/v1/public-check-sessions/token123/submit",
        {
          method: "POST",
