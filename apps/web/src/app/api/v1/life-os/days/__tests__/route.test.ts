...
const {
  ...
  mockFetchLifeOsDaysFromSpring,
  mockCreateLifeOsDayInSpring,
} = vi.hoisted(() => ({
...
vi.mock("@/server/life-os-spring-client", async () => {
  ...
  return {
    ...actual,
    fetchLifeOsDaysFromSpring: (...args: unknown[]) =>
      mockFetchLifeOsDaysFromSpring(...args),
    createLifeOsDayInSpring: (...args: unknown[]) =>
      mockCreateLifeOsDayInSpring(...args),
  };
});
...
describe("api/v1/life-os/days route", () => {
  ...
  it("GET은 Spring 목록 응답을 유지한다", async () => {
    ...
    const response = await GET(
      new NextRequest("http://localhost/api/v1/life-os/days")
    );
  ...
  it("POST는 Spring status를 그대로 노출한다", async () => {
    ...
    mockCreateLifeOsDayInSpring.mockRejectedValue(
      new LifeOsSpringBackendHttpError(409, "중복")
    );
