...
const { requireAuthenticatedUser, mockFetchLifeOsDailyReportFromSpring } =
  vi.hoisted(() => ({
...
vi.mock("@/server/life-os-spring-client", async () => {
  ...
  return {
    ...actual,
    fetchLifeOsDailyReportFromSpring: (...args: unknown[]) =>
      mockFetchLifeOsDailyReportFromSpring(...args),
  };
});
...
describe("api/v1/life-os/reports/daily route", () => {
  ...
  it("GET은 Spring 일간리포트 응답을 유지한다", async () => {
    ...
    mockFetchLifeOsDailyReportFromSpring.mockResolvedValue({
      report: {
        ...
        metrics: {
          ...
          matchedHours: 0,
          overplannedHours: 0,
          restInsteadOfPlanHours: 0,
          unrelatedActionHours: 0,
          spilloverHours: 0,
          overplanningScore: 0,
          mismatchByBlock: { "0-7": 0, "8-15": 0, "16-23": 0 },
          classifications: [],
        },
        patterns: [],
        recommendations: [],
    ...
    const response = await GET(
      new NextRequest(
        "http://localhost/api/v1/life-os/reports/daily?localDate=2026-05-08"
      )
    );
