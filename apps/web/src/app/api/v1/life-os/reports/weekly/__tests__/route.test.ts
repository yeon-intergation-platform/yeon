...
const { requireAuthenticatedUser, mockFetchLifeOsWeeklyReportFromSpring } =
  vi.hoisted(() => ({
...
vi.mock("@/server/life-os-spring-client", async () => {
  ...
  return {
    ...actual,
    fetchLifeOsWeeklyReportFromSpring: (...args: unknown[]) =>
      mockFetchLifeOsWeeklyReportFromSpring(...args),
  };
});
...
describe("api/v1/life-os/reports/weekly route", () => {
  ...
  it("GET은 Spring 주간리포트 응답을 유지한다", async () => {
    ...
    mockFetchLifeOsWeeklyReportFromSpring.mockResolvedValue({
      report: {
        periodType: "weekly",
    ...
    const response = await GET(
      new NextRequest(
        "http://localhost/api/v1/life-os/reports/weekly?periodStart=2026-05-05&periodEnd=2026-05-11"
      )
    );
