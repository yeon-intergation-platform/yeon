...
vi.mock("@/server/cloud-oauth-spring-client", () => ({
  fetchOneDriveOAuthUrlFromSpring: vi.fn(),
...
describe("onedrive auth route", () => {
  ...
  it("비인증이면 shared response를 반환한다", async () => {
    ...
    const response = await GET(new NextRequest("http://localhost/api/v1/integrations/onedrive/auth"));
