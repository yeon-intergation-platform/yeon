...
@Testcontainers
class StudentBoardWriteRepositoryTests {
	private static final UUID OWNER_ID = UUID.fromString("00000000-0000-0000-0000-000000000963");
	...
	@Autowired private StudentBoardWriteRepository repository;
	...
	@Test void snapshot과history를쓴다() {
		var context = repository.findOwnedMemberContext("space_alpha", "mem_1", OWNER_ID);
		assertThat(context).isNotNull();
		repository.upsertBoardSnapshot("smb_1", context.spaceInternalId(), context.memberInternalId(), "present", OffsetDateTime.parse("2026-05-08T01:00:00Z"), "manual", "done", null, OffsetDateTime.parse("2026-05-08T01:00:00Z"), "manual", null, OWNER_ID, OffsetDateTime.parse("2026-05-08T01:00:00Z"));
		var snapshot = repository.findBoardSnapshot(context.spaceInternalId(), context.memberInternalId());
		assertThat(snapshot).isNotNull();
		assertThat(snapshot.attendanceStatus()).isEqualTo("present");
		repository.insertHistory("smbh_1", context.spaceInternalId(), context.memberInternalId(), "present", "done", null, "manual", OWNER_ID, OffsetDateTime.parse("2026-05-08T01:00:00Z"));
		Integer count = jdbcTemplate.queryForObject("select count(*) from public.space_member_board_history", Integer.class);
