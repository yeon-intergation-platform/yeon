...
@Testcontainers
class SheetExportIntegrationRepositoryTests {

	private static final UUID OWNER_ID = UUID.fromString("00000000-0000-0000-0000-000000000807");
	...
	@Autowired private SheetExportIntegrationRepository repository;
	...
	@Test
	void exportIntegration을생성조회갱신삭제한다() {
		Long spaceId = repository.findSpaceInternalId("space_alpha");
		assertThat(spaceId).isNotNull();
		var created = repository.insertExportIntegration(spaceId, "sgi_1", "https://docs.google.com/spreadsheets/d/sheet-1/edit", "sheet-1", OffsetDateTime.parse("2026-05-08T07:00:00Z"));
		assertThat(created.sheetId()).isEqualTo("sheet-1");
		assertThat(repository.findExportIntegration("space_alpha").publicId()).isEqualTo("sgi_1");
		var updated = repository.updateExportIntegration(created.integrationInternalId(), "https://docs.google.com/spreadsheets/d/sheet-2/edit", "sheet-2", OffsetDateTime.parse("2026-05-08T07:10:00Z"));
		assertThat(updated.sheetId()).isEqualTo("sheet-2");
		repository.deleteExportIntegration(spaceId);
		assertThat(repository.findExportIntegration("space_alpha")).isNull();
	}
}
