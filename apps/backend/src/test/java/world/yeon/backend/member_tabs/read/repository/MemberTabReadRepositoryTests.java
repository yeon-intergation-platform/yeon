...
@Testcontainers
class MemberTabReadRepositoryTests {
	...
	void setUpFixture() {
		...
		insertUser(OWNER_ID, "member-tabs-owner@example.com");
		...
		insertTab("mtb_custom", "space_alpha", OWNER_ID, "custom", null, "상담 메모", true, 1);
		insertTab("mtb_hidden", "space_alpha", OWNER_ID, "custom", null, "숨김 탭", false, 2);
		insertTab("mtb_other_space", "space_beta", OWNER_ID, "system", "overview", "다른 스페이스 개요", true, 0);
	}
	...
	@Test
	void space기준탭목록을displayOrder오름차순으로조회한다() {
		Long spaceInternalId = repository.findSpaceInternalId("space_alpha");

		List<MemberTabDefinitionEntity> tabs = repository.findTabsBySpaceInternalId(spaceInternalId);

		assertThat(tabs).extracting(MemberTabDefinitionEntity::getPublicId)
			.containsExactly("mtb_overview", "mtb_custom", "mtb_hidden");
		assertThat(tabs).extracting(MemberTabDefinitionEntity::getDisplayOrder)
