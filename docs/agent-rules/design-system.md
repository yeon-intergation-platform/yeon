...
| 상태 색상    | `text-green` `text-red` `text-rose` `text-cyan`                   |
| 상태 배경    | `bg-green-dim` `bg-red-dim`                                       |
| 상태 보더    | `border-green-border`                                             |
...
**오렌지 계열 금지**: 타이핑/카드/커뮤니티/인증 UI에서 `#e8630a`, `#e87310`, Tailwind `orange-*` 계열을 새로 사용하지 않는다.

---

## 인증 서비스 (auth-credentials) — 다크 중립 테마
...
| CTA       | `#f8f7f3`                | `bg-[#f8f7f3] text-[#080808]`     |
| 보더      | `rgba(255,255,255,0.08)` | `border-[rgba(255,255,255,0.08)]` |

**오렌지 금지**: auth도 오렌지 CTA를 쓰지 않고 중립 CTA만 사용한다.
...
| `features/auth-credentials`                             | 다크(중립)   | auth 정규 hex 목록 |
