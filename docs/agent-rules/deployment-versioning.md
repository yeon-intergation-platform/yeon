...
- 사후 동작 확인은 개발자가 이미 `pnpm dev:all`로 켜둔 `http://localhost:3000/`을 기준으로 한다.
- 에이전트는 `pnpm dev:all`, `pnpm dev`, `next dev`, `expo start` 같은 장기 실행 dev server를 직접 기동하지 않는다.
- `http://localhost:3000/`이 열려 있지 않으면 서버를 켜지 말고, 개발자에게 dev server 기동이 필요하다고 짧게 남긴다.
