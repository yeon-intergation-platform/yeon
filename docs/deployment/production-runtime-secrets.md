...
- 2026-07-11 `production` Environment와 main-only policy 구성을 완료했다.
...
- 환경 파일 자동 로딩을 끈 새 배포에서 전체 컨테이너 재생성, secret mount, digest 고정,
  local/public health와 전체 restart를 검증한 뒤 활성 `/srv/yeon/.env`를 제거했다.
- Repository-level runtime Secrets/Variables도 제거했다. 운영 값은 `production` Environment만
  SSOT이며 백업의 `.env`는 비상 롤백 전용이다.
...
- 활성 `/srv/yeon/.env`가 없는 상태에서 전체 컨테이너 restart와 `yeon.world`,
  `blurt.yeon.world`, `race.yeon.world` 공개 health가 다시 성공함
