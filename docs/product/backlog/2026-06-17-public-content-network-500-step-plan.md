...
논의 필요: 유지보수 대상 서비스 외 공개 콘텐츠를 어디까지 만들지 여부.
선택지: 유지보수 대상만, 별도 보류 섹션, 즉시 포함.
추천: 현재 정책에 맞춰 유지보수 대상 서비스와 공용 계정 문서만 먼저 다룬다.
...
4. **[완료]** Yeon 현재 서비스 목록에서 `typing`, `card`, `community`, `discord-ai`를 추출한다.
...
8. **[완료]** 유지보수 대상 밖 서비스는 공개 콘텐츠 편입 전 별도 결정이 필요하다고 기록한다.
...
350. **[완료]** 유지보수 대상 밖 서비스는 확정 전까지 공개 support 글을 만들지 않는다.
