...
211. 공지 목록은 서비스 필터를 제공한다.
212. 업데이트 목록은 서비스 필터를 제공한다.
213. 업계 뉴스 목록은 주제 필터를 제공한다.
214. 공지 detail에는 `무엇이 바뀌었나요` 섹션을 둔다.
215. 공지 detail에는 `사용자에게 영향이 있나요` 섹션을 둔다.
216. 공지 detail에는 `필요한 조치` 섹션을 둔다.
217. 업데이트 detail에는 `변경 전`과 `변경 후`를 둔다.
218. 업데이트 detail에는 관련 support 문서 링크를 둔다.
219. news detail에는 관련 blog 글 링크를 둔다.
220. 외부 업계 뉴스 인용은 짧게 요약하고 출처 링크를 둔다.
221. 원문 복사는 하지 않는다.
222. news 글은 과장된 마케팅 문장을 피한다.
223. news 홈은 `yeon.world`와 같은 정적이고 차분한 톤을 유지한다.
224. 긴 공지는 본문 목차를 제공한다.
225. 보도자료처럼 보이는 문장은 실제 공지일 때만 사용한다.
...
226. blog 홈은 `engineering`, `product`, `devlog`, `essay`를 분명히 나눈다.
227. engineering 글은 기술 선택과 구현 근거를 중심으로 쓴다.
228. product 글은 왜 만들었는지와 사용자 문제를 중심으로 쓴다.
229. devlog 글은 진행 상황과 배운 점을 중심으로 쓴다.
230. essay 글은 짧고 개인적인 관점을 허용한다.
231. blog 홈에는 최신 글 목록을 먼저 보여준다.
232. blog 홈에는 분류별 대표 글을 보여준다.
233. blog 카드에는 분류, 날짜, 읽는 시간을 표시한다.
234. blog detail에는 작성자 또는 운영 주체를 표시한다.
235. blog detail에는 관련 support 문서 링크를 둔다.
236. blog detail에는 관련 news 공지 링크를 둔다.
237. code block은 engineering 글에서만 주요 요소로 쓴다.
238. 제품 제작기에는 스크린샷을 선택적으로 허용한다.
239. 이미지 없는 글도 어색하지 않게 타이포그래피 중심으로 설계한다.
240. 글 제목은 추상적 감성 문장보다 검색 가능한 구체 문장을 우선한다.
241. 개발 회고 글은 실패와 결정 근거를 숨기지 않는다.
242. 제품 철학 글은 실제 기능과 연결한다.
243. engineering 글은 repo 근거를 링크한다.
244. blog 홈은 과도한 hero 영역을 만들지 않는다.
245. blog detail의 본문 폭은 support보다 약간 넓혀도 된다.
246. 목차는 긴 engineering 글에만 기본 노출한다.
247. essay 글은 목차를 기본 숨김으로 둔다.
248. RSS 또는 feed 제공 여부를 검토한다.
...
326. `타자연습을 시작하는 방법` 글을 작성한다.
327. `타자방에 입장하는 방법` 글을 작성한다.
328. `타자 레이스가 시작되지 않을 때 해결 방법` 글을 작성한다.
329. `타자 결과가 저장되지 않을 때 확인할 것` 글을 작성한다.
330. `typing.yeon.world가 열리지 않을 때 해결 방법` 글을 작성한다.
331. `플래시카드 덱을 만드는 방법` 글을 작성한다.
332. `카드를 추가하고 수정하는 방법` 글을 작성한다.
333. `카드 학습을 시작하는 방법` 글을 작성한다.
334. `카드 데이터가 보이지 않을 때 확인할 것` 글을 작성한다.
335. `card.yeon.world가 열리지 않을 때 해결 방법` 글을 작성한다.
336. `커뮤니티에 글을 쓰는 방법` 글을 작성한다.
337. `커뮤니티에서 댓글을 남기는 방법` 글을 작성한다.
338. `커뮤니티 글이 보이지 않을 때 확인할 것` 글을 작성한다.
339. `community.yeon.world가 열리지 않을 때 해결 방법` 글을 작성한다.
340. `커뮤니티 이용 정책` 글을 작성한다.
341. `YEON 계정으로 로그인하는 방법` 글을 작성한다.
342. `로그인이 풀릴 때 확인할 것` 글을 작성한다.
343. `개인정보는 어떻게 처리되나요` 글을 작성한다.
344. `서비스별 공개 URL과 접속 주소` 글을 작성한다.
345. `오류를 신고하는 방법` 글을 작성한다.
346. 각 typing 글에 레이스 서버 연결 여부 확인 단계를 넣는다.
347. 각 card 글에 게스트와 로그인 사용자 차이를 설명한다.
348. 각 community 글에 공개성과 사용자 행동 기준을 설명한다.
349. account 글은 특정 서비스보다 Yeon 전체 정책 기준으로 작성한다.
350. mooddesk는 유지보수 대상 확정 전까지 공개 support 글을 만들지 않는다.
