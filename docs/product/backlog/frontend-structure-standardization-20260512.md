...
## 88차 — 커뮤니티 댓글 서버 상태 React Query 전환

### 작업내용

- `use-community-feed.ts`의 댓글 목록 서버 상태를 `communityQueryKeys.feedReplies(postId)` 기반 React Query 캐시로 전환한다.
- 댓글 목록은 펼쳐진 게시글 ID 목록에서 파생하고, 댓글 draft/펼침 여부/검증 오류는 local UI state로 유지한다.
- 댓글 작성/삭제 성공 시 해당 게시글의 댓글 목록 key를 명시 무효화하고 게시글 목록의 `replyCount` 캐시 보정은 유지한다.
- 직접 fetch는 추가하지 않고 기존 `chatServiceApi` wrapper만 사용한다.

### 논의 필요

- 펼친 댓글 목록을 닫을 때 캐시를 즉시 삭제할지 여부.

### 선택지

1. 닫아도 React Query 캐시는 유지하고 stale/cache 정책에 맡긴다.
2. 닫을 때 해당 댓글 목록 캐시를 remove한다.

### 추천

- 1번. 펼침은 화면 상태이고 서버 원본 캐시 폐기 조건이 아니므로 닫기만으로 캐시를 지우지 않는다.
