...
> 이 문서는 타자 지문을 외우는 초기 가설의 기록이다. 현재 제품 정의는
> `2026-07-11-card-deck-baekji-recall-2.md`가 진실의 원천이다. 백지는 카드 덱의
> 질문(front)을 보고 답(back)을 전부 쓰는 학습이며, 빈 화면에서 문장 전체를
> 재생하는 흐름은 더 이상 구현 대상이 아니다.
