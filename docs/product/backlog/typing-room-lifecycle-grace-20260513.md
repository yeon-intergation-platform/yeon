...
## 4차: room 화면 내부 명시적 이동 보강

### 작업내용

- 타자방 대기 화면 안에서 다른 내부 페이지 링크를 클릭하는 경우도 explicit leave로 처리한다.
- 브라우저 새로고침/탭 종료처럼 목적지를 확정할 수 없는 unload는 temporary disconnect 정책을 유지한다.

### 논의 필요

- 없음. 사용자가 명시적 이동과 temporary disconnect를 구분하도록 요구했다.

### 선택지

1. 모든 unmount를 explicit leave로 처리한다.
2. 클릭된 내부 링크가 명확할 때만 explicit leave로 처리한다.

### 추천

- 2번. 모든 unmount를 explicit으로 바꾸면 새로고침/네트워크 끊김도 삭제 위험이 다시 생긴다.
