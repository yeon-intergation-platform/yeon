...
- **사용자 방향**: 추천대로 진행.
- **상태**: 구현 완료, 운영 배포 재검증 중. 최초 `main` 실행의 21,000자 표현식 제한은 컨텍스트
  값을 step `env`로 옮겨 해결했다. 후속 실행에서는 GitHub Runner가 확장자 없는 job-start hook
  경로를 거부했다. hook 설치 경로를 `.sh`로 통일하고 실패 job만 재실행한다. 과거 `yeon-ci`
  runner/계정은 제거 완료.
