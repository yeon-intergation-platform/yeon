...
- **사용자 방향**: 추천대로 진행.
- **상태**: 구현 완료, 운영 배포 재검증 중. 최초 `main` 실행은 거대한 deploy `run` 블록에
  Actions 표현식이 포함되어 GitHub의 21,000자 표현식 제한에 걸렸다. 컨텍스트 값은 step `env`로
  전달하고 셸 본문은 일반 환경변수만 사용하도록 후속 수정한다. 과거 `yeon-ci` runner/계정은
  제거 완료.
