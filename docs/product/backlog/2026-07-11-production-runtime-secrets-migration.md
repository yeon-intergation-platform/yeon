...
- `main`은 PR 경유, 최신 branch, SSOT check, conversation resolve를 강제하고 force-push와 삭제를
  금지한다.
- 운영 값은 `production` Environment Secrets/Variables에 값 노출 없이 동기화했고,
  Repository-level 중복 원천은 제거했다.
...
- 활성 `/srv/yeon/.env`는 새 배포와 재시작 검증 뒤 제거했다. 복구 전용 백업만 남아 있다.
...
- **사용자 방향**: 추천대로 진행.
- **상태**: 완료. 서비스별 `/run/secrets` 파일과 raw secret metadata 부재를 운영에서 확인했다.
...
- **사용자 방향**: 추천대로 진행.
- **상태**: 완료. 최초 `main` 실행의 21,000자 표현식 제한과 runner hook `.sh` 확장자 누락을
  수정했다. 실패 job 재실행에서 운영 배포가 성공했고 과거 `yeon-ci` runner/계정도 제거했다.
...
- **사용자 방향**: 추천대로 진행.
- **상태**: 완료. 전체 recreate/restart와 공개 health 후 `/srv/yeon/.env`를 제거했다. 백업은
  `/srv/yeon/backups/secret-migration/20260710T171840Z`에 권한 `700/600`으로 보존한다.
