...
225. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/markdown-content.tsx:495` — 원칙 `D`. 코드 언어 select의 optgroup 생성을 `createYeonDomElement(htmlDocument, "optgroup")`로 바꿔 호출부가 브라우저 전역 `document` 대신 rich DOM 포트에 의존하게 했다. 근거: `const optionGroup = createYeonDomElement(htmlDocument, "optgroup");`
226. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/markdown-content.tsx:499` — 원칙 `D`. 코드 언어 option 생성을 `createYeonDomElement(htmlDocument, "option")`로 바꿔 동일 owner document 기반 포트 생성으로 통일했다. 근거: `const option = createYeonDomElement(htmlDocument, "option");`
227. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:78` — 원칙 `D`. 본문 cursor 이전값 조회를 `CardResizePointerDragPort`로 이동해 `PlayCard`가 `document.body` 구체 구현에 직접 의존하지 않게 했다. 근거: `CARD_RESIZE_POINTER_DRAG_PORT.beginDrag(`
228. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:79` — 원칙 `D`. 본문 userSelect 이전값 조회를 `CardResizePointerDragPort`로 이동해 resize 드래그 런타임 상태를 포트 내부로 숨겼다. 근거: `const previousUserSelect = browserDocument.body.style.userSelect;`
229. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:81` — 원칙 `D`. 본문 cursor 변경을 포트 내부 구현으로 캡슐화해 컴포넌트는 드래그 시작만 위임한다. 근거: `browserDocument.body.style.cursor = "nwse-resize";`
230. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:82` — 원칙 `D`. 본문 userSelect 변경을 포트 내부 구현으로 캡슐화해 컴포넌트는 브라우저 전역 스타일을 직접 다루지 않는다. 근거: `browserDocument.body.style.userSelect = "none";`
231. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:95` — 원칙 `D`. 본문 cursor 복구를 포트의 `stopDrag` 정리 경계로 이동해 자원 정리 책임을 한곳에 모았다. 근거: `browserDocument.body.style.cursor = previousCursor;`
232. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:96` — 원칙 `D`. 본문 userSelect 복구를 포트의 `stopDrag` 정리 경계로 이동했다. 근거: `browserDocument.body.style.userSelect = previousUserSelect;`
233. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:97` — 원칙 `D`. pointermove listener 제거를 포트 정리 함수로 이동해 컴포넌트가 `window`에 직접 의존하지 않게 했다. 근거: `browserWindow.removeEventListener("pointermove", handlePointerMove);`
234. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:98` — 원칙 `D`. pointerup listener 제거를 포트 정리 함수로 이동했다. 근거: `browserWindow.removeEventListener("pointerup", stopDrag);`
235. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:99` — 원칙 `D`. pointercancel listener 제거를 포트 정리 함수로 이동했다. 근거: `browserWindow.removeEventListener("pointercancel", stopDrag);`
236. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:102` — 원칙 `D`. pointermove listener 등록을 포트 내부로 이동해 resize 드래그 이벤트 바인딩을 추상화했다. 근거: `browserWindow.addEventListener("pointermove", handlePointerMove);`
237. **[완료][P3] 브라우저 전역 직접 의존 점검** `apps/web/src/features/card-service/components/play-card.tsx:103` — 원칙 `D`. pointerup listener 등록을 포트 내부로 이동했다. 근거: `browserWindow.addEventListener("pointerup", stopDrag);`
...
242. **[완료][P3] 브라우저 전역 직접 의존 점검** `packages/ui/src/hooks/YeonBrowserHooks/index.ts:9` — 원칙 `D`. 본문 overflow 이전값 조회를 `YEON_BROWSER_HOOKS_PORT.body.getStyleProperty`로 감싸 hook이 `document.body`에 직접 의존하지 않게 했다. 근거: `YEON_BROWSER_HOOKS_PORT.body.getStyleProperty("overflow")`
243. **[완료][P3] 브라우저 전역 직접 의존 점검** `packages/ui/src/hooks/YeonBrowserHooks/index.ts:10` — 원칙 `D`. 본문 overflow 잠금을 `YEON_BROWSER_HOOKS_PORT.body.setStyleProperty`로 이동해 브라우저 body style 변경을 포트에 위임했다. 근거: `YEON_BROWSER_HOOKS_PORT.body.setStyleProperty("overflow", "hidden")`
244. **[완료][P3] 브라우저 전역 직접 의존 점검** `packages/ui/src/hooks/YeonBrowserHooks/index.ts:13` — 원칙 `D`. 본문 overflow 복구를 동일 포트로 이동해 scroll lock 자원 정리 경계를 포트 중심으로 통일했다. 근거: `YEON_BROWSER_HOOKS_PORT.body.setStyleProperty(`
245. **[완료][P3] 브라우저 전역 직접 의존 점검** `packages/ui/src/hooks/YeonBrowserHooks/index.ts:22` — 원칙 `D`. 본문 class 추가를 `YEON_BROWSER_HOOKS_PORT.body.addClass`로 감싸 hook이 body classList 구현에 직접 의존하지 않게 했다. 근거: `YEON_BROWSER_HOOKS_PORT.body.addClass(className)`
246. **[완료][P3] 브라우저 전역 직접 의존 점검** `packages/ui/src/hooks/YeonBrowserHooks/index.ts:25` — 원칙 `D`. 본문 class 제거를 `YEON_BROWSER_HOOKS_PORT.body.removeClass`로 감싸 class 정리 경계를 포트 중심으로 통일했다. 근거: `YEON_BROWSER_HOOKS_PORT.body.removeClass(className)`
247. **[완료][P3] 브라우저 전역 직접 의존 점검** `packages/ui/src/hooks/YeonBrowserHooks/index.ts:44` — 원칙 `D`. document event 등록을 `YEON_BROWSER_HOOKS_PORT.events.subscribeDocument`로 감싸 호출부가 `document.addEventListener`에 직접 의존하지 않게 했다. 근거: `subscribeDocument(type, listener)`
248. **[완료][P3] 브라우저 전역 직접 의존 점검** `packages/ui/src/hooks/YeonBrowserHooks/index.ts:45` — 원칙 `D`. document event 해제를 포트의 unsubscribe 반환값으로 통일했다. 근거: `return () => browserDocument.removeEventListener(type, listener);`
249. **[완료][P3] 브라우저 전역 직접 의존 점검** `packages/ui/src/hooks/YeonBrowserHooks/index.ts:66` — 원칙 `D`. hook 기반 document event 등록도 동일 `subscribeDocument` 포트로 통일했다. 근거: `return YEON_BROWSER_HOOKS_PORT.events.subscribeDocument(type, listener);`
...
289. **[완료][P3] 상태 분기 매핑/전략화** `apps/race-server/src/rooms/card-room.ts:58` — 원칙 `O`. WebRTC SDP 입력 검증을 공용 `isNonEmptyString` type guard로 통일했다. 근거: `if (!isNonEmptyString(obj.sdp)) return null;`
...
- 항목 281 완료: 카드 학습 상세 조회 source 판정을 `createCardDeckPlayDetailReader`로 분리했다.
...
- 항목 289 완료: WebRTC SDP 입력 검증을 공용 `isNonEmptyString` type guard로 통일했다.
