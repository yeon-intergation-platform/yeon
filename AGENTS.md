...
모든 코드 작성·수정·리뷰에서 아래 질문을 항상 적용한다.

- 이 클래스/함수는 한 가지 책임만 가지는가? (단일 책임 원칙, SRP)
- 새 기능을 추가할 때 기존 코드를 덜 고치고 확장할 수 있는가? (개방-폐쇄 원칙, OCP)
- 자식 클래스를 부모 타입 자리에 넣어도 문제없이 동작하는가? (리스코프 치환 원칙, LSP)
- 인터페이스가 필요 없는 기능까지 강요하지 않는가? (인터페이스 분리 원칙, ISP)
- 구체 클래스보다 인터페이스/추상화에 의존하는가? (의존성 역전 원칙, DIP)
- 같은 로직이 반복되고 있지 않은가? (DRY 원칙)
- 괜히 복잡하게 짜지 않았는가? (KISS 원칙)
- 아직 필요 없는 기능을 미리 만들고 있지 않은가? (YAGNI 원칙)
- 역할별로 코드가 잘 분리되어 있는가? (관심사 분리 원칙, SoC)
- 관련 있는 코드는 가까이 모여 있는가? (높은 응집도 원칙)
- 서로 다른 코드끼리 너무 강하게 묶여 있지 않은가? (낮은 결합도 원칙)
- 내부 상태를 아무나 바꾸지 못하게 캡슐화했는가? (캡슐화 원칙)
- 상속보다 조합으로 해결할 수 있지 않은가? (상속보다 조합 원칙)
- 이름만 봐도 변수/함수/클래스의 의미가 보이는가? (명확한 이름 짓기 원칙)
- 함수 하나가 너무 많은 일을 하고 있지 않은가? (작은 함수 원칙)
- 조회 함수가 몰래 데이터를 변경하고 있지 않은가? (명령-조회 분리 원칙, CQS)
- 코드가 이름과 다르게 예상 밖 행동을 하지 않는가? (최소 놀람 원칙)
- 외부 입력값을 검증하고 있는가? (입력 검증 원칙)
- null, 빈 값, 잘못된 값 같은 경계 상황을 고려했는가? (경계값 처리 원칙)
- 예외를 처리할 수 없는 곳에서 억지로 잡고 있지 않은가? (처리 가능한 예외만 잡기 원칙)
- Exception으로 너무 넓게 뭉뚱그려 잡고 있지 않은가? (구체적 예외 처리 원칙)
- 예외를 조용히 숨기고 있지 않은가? (예외 은닉 금지 원칙)
- 예외 메시지만 봐도 원인 추적이 가능한가? (의미 있는 예외 메시지 원칙)
- 예외를 일반 if문 대용으로 남용하고 있지 않은가? (예외 남용 금지 원칙)
- 하위 기술 예외를 필요한 경우 비즈니스 예외로 변환했는가? (예외 변환 원칙, Exception Translation)
- 예외를 변환할 때 원래 예외 정보를 보존했는가? (원인 예외 보존 원칙)
- 파일/DB/네트워크 자원을 예외가 나도 정리하는가? (자원 정리 원칙)
- 공통 예외 처리를 한 곳에서 일관되게 처리하는가? (전역 예외 처리 원칙)
- 잘못된 값은 최대한 빨리 터뜨리고 있는가? (Fail Fast 원칙)
- 이 코드는 테스트하기 쉬운가? (테스트 가능성 원칙)
- 실제 DB/API 없이도 가짜 객체로 테스트할 수 있는가? (의존성 주입 원칙, DI)
- 변경 가능한 상태를 최대한 줄였는가? (불변성 선호 원칙)
- 주석 없이도 코드 흐름이 읽히는가? (자기 설명적 코드 원칙)
- 주석이 코드의 변명을 하는 게 아니라 의도를 설명하는가? (좋은 주석 원칙)
- 나중에 내가 다시 봐도 이해할 수 있는가? (가독성 원칙)
- 코드를 고친 뒤 처음보다 조금이라도 더 깨끗해졌는가? (보이스카우트 원칙)
